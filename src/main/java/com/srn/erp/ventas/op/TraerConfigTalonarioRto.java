...
public class TraerConfigTalonarioRto extends Operation {

  public TraerConfigTalonarioRto() {
  }
  ...
  private void cargarOperaciones(IDataSet datasetOperHabilitadas,IDataSet datasetCompoStock,Talonario talonario) throws BaseException {
    int oid = 0;
    Iterator iterTiposRtos = RemitoCab.getTipos().keySet().iterator();
    while (iterTiposRtos.hasNext()) {
      String codigo      = (String) iterTiposRtos.next();
      String descripcion = (String) RemitoCab.getTipos().get(codigo);
