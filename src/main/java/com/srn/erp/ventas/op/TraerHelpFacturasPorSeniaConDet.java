...
public class TraerHelpFacturasPorSeniaConDet extends Operation {

  public TraerHelpFacturasPorSeniaConDet() {
  }
  ...
  private void traerFacturas(
    ...
    if (mapaDatos.containsKey("oc"))
        oc = mapaDatos.getString("oc");
    
    

    List listaFacturas =
        FacturaCab.getFacturasPorSeniaoAnt(nroExtDesde,nroExtHasta,fecDesde,fecHasta,sujeto,producto,oc,getSesion());
  ...
  private void cargarRegistroFactura(IDataSet dataset,
    ...
    dataset.fieldValue("moneda", factura.getMoneda().getSimbolo());
    dataset.fieldValue("activo", factura.isActivo());
    dataset.fieldValue("oc", factura.getOC());
