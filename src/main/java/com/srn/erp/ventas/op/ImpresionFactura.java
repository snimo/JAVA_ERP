...
public class ImpresionFactura extends Operation {
	...
	private IDataSet getDataSetFacturaCab() {
		...
		dataset.fieldDef(new Field("cae", Field.STRING, 100));
		dataset.fieldDef(new Field("vto_cae", Field.STRING, 100));
	...
	private void cargarRegistroFacturaCab(IDataSet dataset, FacturaCab facturaCab) throws BaseException {
		...
		Empresa empresa = Empresa.findByOidProxy(this.getSesion().getLogin().getOidEmpresa(), this.getSesion());
		Talonario talonario = facturaCab.getTalonario();
		talonario.supportRefresh();
		
		
		if (talonario.isFactElec()) {
			if ((facturaCab.getCaeFE()!=null) && (facturaCab.getCaeFE().length()>0))
				talonario.setCai(facturaCab.getCaeFE());
			if (facturaCab.getFecVtoCaeFE()!=null)
				talonario.setFec_vto(facturaCab.getFecVtoCaeFE());		
		}
		
				
		dataset.fieldValue("codigo_barra", talonario.getCodigoBarraImpresionFactura(empresa.getCuit()));
		...
		String cae = "";
		if (facturaCab.getCaeFE()!=null) cae = "Nro. CAE "+facturaCab.getCaeFE();
		
		String vtoCAE = "";
		if (facturaCab.getFecVtoCaeFE()!=null) vtoCAE = "Vto. CAE "+Fecha.getddmmyyyy(facturaCab.getFecVtoCaeFE());
		dataset.fieldValue("cae",cae);
		dataset.fieldValue("vto_cae",vtoCAE);
