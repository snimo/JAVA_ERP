...
public class TraerTableroProgramado extends Operation {

	private String nombreTabla = null;
	private boolean soloTurnosLibres = false;
	private boolean soloTurnosActivos = false;

	public TraerTableroProgramado() {
	}
	...
	public void execute(MapDatos mapaDatos) throws BaseException {
		...
		if (mapaDatos.containsKey("SOLO_TURNOS_LIBRES"))
			soloTurnosLibres = true;
		else if (mapaDatos.containsKey("SOLO_TURNOS_ACTIVOS"))
			 soloTurnosActivos = true;
	...
	private void cargaTurnos(IDataSet ds, TableroTurnos tablero,
		...
		int i = 1;
		Iterator iterHorarios = null;
		if (soloTurnosLibres)
			iterHorarios = tablero.getHorariosLibresActivos(entidad).iterator();
		else if (soloTurnosActivos)
			iterHorarios = tablero.getHorariosActivos(entidad).iterator();
			else
				iterHorarios = entidad.getHorarios().iterator();
