...
public class ImpresionEvaluacionLegajo extends Operation {

	public ImpresionEvaluacionLegajo() {
	}
	...
	public void execute(MapDatos mapaDatos) throws BaseException {
		...
		writeCliente(dsEvalCab);
		writeCliente(dsCompetencias);
		writeCliente(dsAspectosDest);
		writeCliente(dsAspectosMejorar);
		writeCliente(dsComenEval);
	...
	private IDataSet getDataSetComenEval() {
		IDataSet dataset = new TDataSet();
		dataset.create("TComentarioEvaluado");
		dataset.fieldDef(new Field("oid_eval_cab", Field.INTEGER, 0));
		dataset.fieldDef(new Field("comentario", Field.MEMO, 0));
		return dataset; 
	}
	
	private void cargarComentarioEvaluado(IDataSet ds,String comentario , EvaluacionLegajoCab evaluacion) throws BaseException {
		
		if ((comentario!=null) && (comentario.length()!=0)) {
			ds.append();
			ds.fieldValue("oid_eval_cab" , evaluacion.getOIDInteger());
			ds.fieldValue("comentario", comentario);
		}
	...
	private void cargarEvaluacionCab(IDataSet dsEvaluacionCab, EvaluacionLegajoCab aEvaluacionLegajoCab) throws BaseException {
		...
		dsEvaluacionCab.fieldValue("modelo", aEvaluacionLegajoCab.getModelo_eval_cab().getDesc_abreviada()+ " del "+
		Fecha.getddmmyyyy(aEvaluacionLegajoCab.getModelo_eval_cab().getFec_desde())+" al "+
		Fecha.getddmmyyyy(aEvaluacionLegajoCab.getModelo_eval_cab().getFec_hasta()));
	  ...
	  private IDataSet getDataSetEvaluadores() { 
		    ...
		    dataset.fieldDef(new Field("firmado", Field.BOOLEAN, 0));
		    dataset.fieldDef(new Field("desc_firmado", Field.STRING, 2));
		    dataset.fieldDef(new Field("fec_firmado", Field.DATE, 0));
		    dataset.fieldDef(new Field("activo", Field.BOOLEAN, 0));
	  ...
	  private void cargarRegistrosEvaluadores(IDataSet dsEvaluadores,EvaluacionLegajoCab evaluacionLegajoCab) throws BaseException {
		  ...
		  while (iterEvaluadores.hasNext()) {
			  ...
			  if (evalObsGen.isFirmado())
				  dsEvaluadores.fieldValue("desc_firmado", "Si");
			  else
				  dsEvaluadores.fieldValue("desc_firmado", "No");
		  }
	  }
	  ...
	  private void cargarRegistrosObsGenRRHH(IDataSet dsObsGenRRHH,EvaluacionLegajoCab evaluacionLegajoCab) throws BaseException {
		  ...
		  while (iterObsCodif.hasNext()) {
			  ...
			  --secu;
			  dsObsGenRRHH.append();
			  dsObsGenRRHH.fieldValue("oid_obs_gen", secu);
			  dsObsGenRRHH.fieldValue("oid_eval_cab", evaluacionLegajoCab.getOIDInteger());
			  dsObsGenRRHH.fieldValue("observacion", obsCodifEvalRH.getDescripcion());
			  dsObsGenRRHH.fieldValue("activo", obsCodifEvalRH.isActivo());
	  ...
	  private void cargarRegistrosFirmas(IDataSet dsFirmas,EvaluacionLegajoCab evaluacionLegajoCab) throws BaseException {
		  ...
		  while (iterFirmas.hasNext()) {
			  ...
			  if (evalFirma.isFirmo())
				  dsFirmas.fieldValue("desc_firmado", "Si");
			  else
				  dsFirmas.fieldValue("desc_firmado", "No");
