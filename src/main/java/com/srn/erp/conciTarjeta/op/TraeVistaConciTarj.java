...
public class TraeVistaConciTarj extends Operation { 

  int secu = 0;
	
  public TraeVistaConciTarj() { 
  }
  ...
  public void execute(MapDatos mapaDatos) throws BaseException {
	  ...
	  IDataSet dsVistaVert = this.getDSVistaVertConci();
		...
		while (!ds.EOF()) {
			...
			if (iterDetCupo.hasNext())
				detCupon = (ArchLoteConciDet) iterDetCupo.next();
						
			boolean procUltRegSolo = false;
			
			while ((procUltRegSolo) || (iterDetCupo.hasNext())) {

				nroConci = detCupon.getNroConciliacion();
				cuponesPOSNET.clear();
				
				if (procUltRegSolo) { 
					procUltRegSolo = false;
					cuponesPOSNET.add(detCupon);
				}
				
				
				while ((iterDetCupo.hasNext() && (nroConci == detCupon.getNroConciliacion().intValue()))) {
					
					cuponesPOSNET.add(detCupon);
					
					detCupon = (ArchLoteConciDet) iterDetCupo.next();

					// Verificar si se trata del ultimo registro
					if (iterDetCupo.hasNext()==false) {
						if (nroConci == detCupon.getNroConciliacion().intValue())
							cuponesPOSNET.add(detCupon);
						else 
							procUltRegSolo = true;
					}
				...
				if (cuponesPOSNET.size()>cuponesTARJ.size()) {
					...
					while (iterCupPOSNET.hasNext()) {
						...
						cargarRegistro(dsVistaVert, 
								...
								archLoteConciDet1.isPresentado(),
								archLoteConciDet1.isMarcarPreconci(),
								archLoteConciDet1.isPreconciDudoso(),
								nroConciAut
								);
						++i;
					}
					...
					while (iterCupTARJ.hasNext()) {
						...
						cargarRegistro(dsVistaVert, 
								...
								false,
								detTarConciEnt1.isMarcarPreconci(),
								detTarConciEnt1.isPreconciDudoso(),
								nroConciAut
								);
						++i;
					}
	  ...
	  if (pendConci) {	
		  ...
		  while (iterDetTar.hasNext()) {
			  ...
			  if (detTarConciEnt.getCabConciAut()!=null)
				  nroConciAut = detTarConciEnt.getCabConciAut().getOIDInteger(); 
			  cargarRegistro(dsVistaVert, 
						   	null, 
						   	detTarConciEnt, 
						   	detTarConciEnt.isPreConciliado(), 
						   	detTarConciEnt.getFecPreconci(), 
						   	detTarConciEnt.isConciliado(), 
						   	detTarConciEnt.getFecConciliado(), 
						   	detTarConciEnt.getNroConciliacion(), true ,
						   	false,
						   	detTarConciEnt.isMarcarPreconci(),
						   	detTarConciEnt.isPreconciDudoso(),
						   	nroConciAut
						   	);			
		  }
	  }
	...
	private void cargarRegistro(
		...
		dataset.fieldValue("marca", marca);
		if (dudoso)
			dataset.fieldValue("dudoso","?");
		else
			dataset.fieldValue("dudoso","");
		dataset.fieldValue("nro_conci_aut", nroConciAut);
		
		if (archLoteConciDet!=null) 
			dataset.fieldValue("cupon_perdido", archLoteConciDet.isCuponPerdido());
		else
			dataset.fieldValue("cupon_perdido", false);
		if (archLoteConciDet!=null)
			dataset.fieldValue("est_asi_cob", archLoteConciDet.getEstadoAsientoCobranza());
		else
			dataset.fieldValue("est_asi_cob", "");
		
		if (archLoteConciDet!=null) {
			if (archLoteConciDet.getAsientoACobrar()!=null)
				dataset.fieldValue("oid_cab_asi_acob", archLoteConciDet.getAsientoACobrar().getOID());
			else
				dataset.fieldValue("oid_cab_asi_acob", "");
		}
		else
			dataset.fieldValue("oid_cab_asi_acob", "");
		
		if (archLoteConciDet!=null)
			dataset.fieldValue("est_asi_liq", archLoteConciDet.getEstadoAsientoLiquidacion());
		else
			dataset.fieldValue("est_asi_liq", "");
		
		if (archLoteConciDet!=null) {
			if (archLoteConciDet.getCabAsiLiquiTarj()!=null)
				dataset.fieldValue("oid_cab_asi_liq", archLoteConciDet.getCabAsiLiquiTarj().getOID());
			else
				dataset.fieldValue("oid_cab_asi_liq", "");
		}
		else
			dataset.fieldValue("oid_cab_asi_liq", "");
		
		if (archLoteConciDet!=null)
			dataset.fieldValue("ref_asi_cob", archLoteConciDet.getRefSistExt());
		else
			dataset.fieldValue("ref_asi_cob", "");
		
		if (archLoteConciDet!=null)
			dataset.fieldValue("ref_asi_liq", archLoteConciDet.getRefSistExt1());
		else
			dataset.fieldValue("ref_asi_liq", "");		
