package com.srn.erp.pagos.da;

import java.sql.*;
import java.util.List;

import com.srn.erp.general.bm.PeriodoInc;
import com.srn.erp.general.bm.PeriodoIncDet;
import com.srn.erp.general.da.DBPeriodoIncDet;
import com.srn.erp.pagos.bm.*;

import framework.applicarionServer.bl.ObjetosAplicacion.ObjetoLogico;
import framework.applicarionServer.bl.Persistencia.*;
import framework.applicarionServer.bl.Sesion.ISesion;
import framework.applicarionServer.bl.Utils.ListObserver;
import framework.request.bl.Utils.*;

public class DBRendicionFFImpuestos extends DBObjetoPersistente {

  public static final String OID_CONC_IMPU_FP = "oid_conc_impu_fp";
  public static final String OID_REND_FF_DET = "oid_rend_ff_det";
  public static final String OID_CONC_IMPU = "oid_conc_impu";
  public static final String IMPORTE = "importe";
  public static final String COMPORTAMIENTO = "comportamiento";
  
  public static final int SELECT_BY_REND_FF_DET = 100;

  public DBRendicionFFImpuestos() { 
  }

  protected void insert(IObjetoPersistente objetoPersistente) throws BaseException, SQLException { 

    final int OID_CONC_IMPU_FP = 1;
    final int OID_REND_FF_DET = 2;
    final int OID_CONC_IMPU = 3;
    final int IMPORTE = 4;
    final int COMPORTAMIENTO = 5;

    RendicionFFImpuestos pers = (RendicionFFImpuestos) objetoPersistente;
    
    StringBuffer sqlInsert = new StringBuffer();
    sqlInsert.append(" insert into opRendFFImpuestos "+
                     " ( "+
                      "OID_CONC_IMPU_FP,"+
                      "OID_REND_FF_DET,"+
                      "OID_CONC_IMPU,"+
                      "IMPORTE,"+
                      "COMPORTAMIENTO)"+ 
                      " values "+ 
                      "("+
                      "?,"+
                      "?,"+
                      "?,"+
                      "?,"+
                      "?) ");

    PreparedStatement qInsert = getSesion().getConexionBD().prepareStatement(sqlInsert.toString()); 
    qInsert.setInt(OID_CONC_IMPU_FP,pers.getOID());
    qInsert.setInt(OID_REND_FF_DET,pers.getRend_ff_det().getOID());
    qInsert.setInt(OID_CONC_IMPU,pers.getConcepto_impuesto().getOID());
    qInsert.setDouble(IMPORTE,pers.getImporte().doubleValue());
    qInsert.setString(COMPORTAMIENTO,pers.getComportamiento());
    qInsert.executeUpdate();
    qInsert.close();
  }


  protected void update(IObjetoPersistente objetoPersistente) throws BaseException, SQLException { 

    final int OID_REND_FF_DET = 1;
    final int OID_CONC_IMPU = 2;
    final int IMPORTE = 3;
    final int COMPORTAMIENTO = 4;
    final int OID_CONC_IMPU_FP = 5;

    RendicionFFImpuestos pers = (RendicionFFImpuestos) objetoPersistente;
    
    StringBuffer sqlUpdate= new StringBuffer();
    sqlUpdate.append(" update opRendFFImpuestos set "+
              "oid_rend_ff_det=?"+ 
              ",oid_conc_impu=?"+ 
              ",importe=?"+ 
              ",comportamiento=?"+ 
                 " where " +
                 " oid_conc_impu_fp=? ");

    PreparedStatement qUpdate = getSesion().getConexionBD().prepareStatement(sqlUpdate.toString()); 
    qUpdate.setInt(OID_CONC_IMPU_FP,pers.getOID());
    qUpdate.setInt(OID_REND_FF_DET,pers.getRend_ff_det().getOID());
    qUpdate.setInt(OID_CONC_IMPU,pers.getConcepto_impuesto().getOID());
    qUpdate.setDouble(IMPORTE,pers.getImporte().doubleValue());
    qUpdate.setString(COMPORTAMIENTO,pers.getComportamiento());
    qUpdate.executeUpdate(); 
    qUpdate.close();
  }

  protected void delete(IObjetoPersistente objetoPersistente) throws BaseException, SQLException { 
    final int OID_CONC_IMPU_FP = 1; 
    RendicionFFImpuestos pers = (RendicionFFImpuestos) objetoPersistente;
    StringBuffer sqlUpdate = new StringBuffer(); 
    sqlUpdate.append("update opRendFFImpuestos "+
                     " set activo=0 " + 
                     " where " + 
                     " oid_conc_impu_fp=? ");
    PreparedStatement qUpdate = getSesion().getConexionBD().prepareStatement(sqlUpdate.toString()); 
    qUpdate.setInt(OID_CONC_IMPU_FP, pers.getOID()); 
    qUpdate.executeUpdate();
    qUpdate.close();
  } 

  protected void rehabilitar(IObjetoPersistente objetoPersistente) throws BaseException, SQLException { 
    final int OID_CONC_IMPU_FP = 1; 
    RendicionFFImpuestos pers = (RendicionFFImpuestos) objetoPersistente;
    StringBuffer sqlUpdate = new StringBuffer(); 
    sqlUpdate.append("update opRendFFImpuestos "+
                     " set activo=1 " + 
                     " where " + 
                     " oid_conc_impu_fp=? ");
    PreparedStatement qUpdate = getSesion().getConexionBD().prepareStatement(sqlUpdate.toString()); 
    qUpdate.setInt(OID_CONC_IMPU_FP, pers.getOID()); 
    qUpdate.executeUpdate();
    qUpdate.close();
  } 

  public PreparedStatement prepararSelect(int aSelect, Object aCondiciones) throws BaseException, SQLException {
    PreparedStatement ps = null; 
    switch (aSelect) { 
      case IDBObjetoPersistente.SELECT_BY_OID: { 
        ps = this.selectByOID(aCondiciones); 
        break; 
      } 
      case IDBObjetoPersistente.SELECT_BY_CODIGO: { 
        ps = this.selectByCodigo(aCondiciones); 
        break; 
      } 
      case IDBObjetoPersistente.SELECT_ALL_HELP: { 
        ps = this.selectAllHelp(aCondiciones); 
        break; 
      } 
      case SELECT_BY_REND_FF_DET: {
        ps = this.selectByRendFFDet(aCondiciones); 
        break; 
      }
    } 
    return ps;
  } 

  private PreparedStatement selectByOID(Object aCondiciones) throws BaseException, SQLException { 

    final int OID_CONC_IMPU_FP = 1; 

    StringBuffer textSQL = new StringBuffer(); 
    textSQL.append("SELECT * FROM  opRendFFImpuestos "); 
    textSQL.append(" WHERE oid_conc_impu_fp = ? "); 
    PreparedStatement querySelect = getSesion().getConexionBD().prepareStatement(textSQL.toString()); 
    int oid = ( (Integer) aCondiciones).intValue(); 
    querySelect.setInt(OID_CONC_IMPU_FP, oid); 
    return querySelect; 
  }
  
  private PreparedStatement selectByRendFFDet(Object aCondiciones) throws BaseException, SQLException { 
    StringBuffer textSQL = new StringBuffer(); 
    textSQL.append("SELECT * FROM  opRendFFImpuestos "); 
    textSQL.append(" WHERE oid_rend_ff_det = ? "); 
    PreparedStatement querySelect = getSesion().getConexionBD().prepareStatement(textSQL.toString());
    
    RendicionFondoFijoCompro rendFFCompro = ( (RendicionFondoFijoCompro) aCondiciones); 
    querySelect.setInt(1, rendFFCompro.getOID()); 
    return querySelect; 
  }

  
  private PreparedStatement selectByCodigo(Object aCondiciones) throws BaseException, SQLException { 

    final int CODIGO = 1; 

    StringBuffer textSQL = new StringBuffer(); 
    textSQL.append("SELECT * FROM  opRendFFImpuestos "); 
    textSQL.append(" WHERE  = ? ");
    PreparedStatement querySelect = getSesion().getConexionBD().prepareStatement(textSQL.toString()); 
    String codigo = (String) aCondiciones; 
    querySelect.setString(CODIGO, codigo); 
    return querySelect; 
  }

  private PreparedStatement selectAllHelp(Object aCondiciones) throws BaseException, SQLException {

    StringBuffer textSQL = new StringBuffer();

    textSQL.append("SELECT oid_conc_impu_fp oid, codigo,  descripcion ,activo ");
    textSQL.append(" from opRendFFImpuestos");
    
    PreparedStatement querySelect = getSesion().getConexionBD().prepareStatement(textSQL.toString()); 
    return querySelect; 
  }
  
  public static List getConcImpuRendFF(RendicionFondoFijoCompro rendFFCompro,
      ISesion aSesion) throws BaseException {
   return (List) ObjetoLogico.getObjects(RendicionFFImpuestos.NICKNAME,
         DBRendicionFFImpuestos.SELECT_BY_REND_FF_DET,
         rendFFCompro,
         new ListObserver(),
         aSesion);
  }
  
  
} 
