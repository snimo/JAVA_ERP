...
public class TraerIngMensualPorTCyTV extends Operation {
  ...
  public TraerIngMensualPorTCyTV() {
  }
  ...
  public void execute(MapDatos mapaDatos) throws BaseException {
	  ...
	  while (iteratorReg.hasNext()) {
		  ...
		  cargarRegistroIngDiarioPorTCyTV(ds, ++secu, regCons.moneda, regCons.tipoCompro, regCons.periodo, regCons.tipoValor, regCons.calcTotal.getResult());
	  }
  ...
  private String getClave(TipoComprobante tc,TipoValor tv,Periodo periodo) throws BaseException {
	return   tc.getOIDInteger()+"_"+tv.getOIDInteger()+"_"+periodo.getCodigo();
  }
  
  private void cargar(MovimientoCaja movimientoCaja) throws BaseException {
	  Periodo periodo = Periodo.getPeriodoMensualByFecha(movimientoCaja.getFechacaja(), this.getSesion());
	  String clave = getClave(movimientoCaja.getComprobante().getTipoCompro(), movimientoCaja.getTipovalor(), periodo);
	  ...
	  if (registroConsulta == null) {
		  ...
		  registroConsulta.tipoCompro = movimientoCaja.getComprobante().getTipoCompro().getDescripcion();
		  registroConsulta.tipoValor = movimientoCaja.getTipovalor().getDescripcion();
		  registroConsulta.periodo = periodo.getCodigo();
