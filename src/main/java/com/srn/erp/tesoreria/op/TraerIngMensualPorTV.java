...
public class TraerIngMensualPorTV extends Operation {
  ...
  public void execute(MapDatos mapaDatos) throws BaseException {
	  ...
	  while (iteratorReg.hasNext()) {
		  ...
		  cargarRegistroIngDiarioPorTCyTV(ds, ++secu, regCons.moneda, regCons.periodo, regCons.tipoValor, regCons.calcTotal.getResult());
	  }
  ...
  private IDataSet getDSIngDiarioPorTCyTC() {
    ...
    dataset.create("TIngMensualPorTV");
    dataset.fieldDef(new Field("secu", Field.INTEGER, 0));
    dataset.fieldDef(new Field("moneda", Field.STRING, 50));
    dataset.fieldDef(new Field("periodo", Field.STRING, 50));
  ...
  private String getClave(TipoValor tv,Periodo periodo) throws BaseException {
	return   tv.getOIDInteger()+"_"+periodo.getCodigo();
  }
  ...
  private void cargar(MovimientoCaja movimientoCaja) throws BaseException {
	  ...
	  String clave = getClave( movimientoCaja.getTipovalor(),periodo);
