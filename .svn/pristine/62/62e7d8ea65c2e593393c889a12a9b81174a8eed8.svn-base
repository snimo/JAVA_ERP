...
public class ValidarOCPendRecep extends Operation { 

  public ValidarOCPendRecep() { 
  }
  ...
  public void execute(MapDatos mapaDatos) throws BaseException { 
    ...
    if (identificador.length()==0) throw new ExceptionValidation(null,"Debe ingresar algún dato para localizar la Orden de Compra");
    
    String tc = OrdenDeCompraCab.getTCDefaultBusqueda(this.getSesion()).getCodigo();
    String letra = OrdenDeCompraCab.getLetraDefaultBusqueda(this.getSesion());
    Integer lugEmi =  OrdenDeCompraCab.getLugEmiDefaultBusqueda(this.getSesion()).getLugemi();
    
    IDataSet dsValOC = getDataSetOrdenDeCompraCab();
    
    List listOC =
    	OrdenDeCompraCab.getOrdenesDeCompraPendRecep(proveedor,tc,letra,lugEmi,identificador,this.getSesion());
    
    
    if (listOC.isEmpty())
    	throw new ExceptionValidation(null,"No se encontraron Ordenes de Compra:"+identificador);
    
    Iterator iterOC = listOC.iterator();
    while (iterOC.hasNext()) {
    	OrdenDeCompraCab oc = (OrdenDeCompraCab) iterOC.next();
    	cargarRegistroOrdenDeCompraCab(dsValOC,oc);
    }
