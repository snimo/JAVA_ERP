...
public class ModifMovManuTARJETA extends Operation {

	EmpresaConciTar empresa = null;

	public ModifMovManuTARJETA() {
	}

	public void execute(MapDatos mapaDatos) throws BaseException {

		DetTarConciEnt detTarConciEnt = DetTarConciEnt.findByOidProxy(mapaDatos.getInteger("oid_tar_info_det"), this.getSesion());
		empresa = detTarConciEnt.getEmpresa();

		if (empresa == null)
			throw new ExceptionValidation(null, "Debe seleccionar la empresa");
		
		if (detTarConciEnt == null)
			throw new ExceptionValidation(null,"Debe seleccionar un Movimiento de TARJETA");
		...
		if (detTarConciEnt.isPreConciliado())
			throw new ExceptionValidation(null,"No se puede modificar un Movimiento de TARJETA que se encuentra conciliado");
		
		if (detTarConciEnt.isConciliado())
			throw new ExceptionValidation(null,"No se puede modificar un Movimiento de TARJETA que se encuentra confirmado");		
		
		IDataSet dataset = mapaDatos.getDataSet("TInput");
		procesarRegistros(dataset, detTarConciEnt);
	}

	private void procesarRegistros(IDataSet dataset, DetTarConciEnt detTarConciEnt) throws BaseException {
		dataset.first();
		while (!dataset.EOF()) {

			TarjetaConciTar tarjeta = TarjetaConciTar.findByOidProxy(dataset.getInteger("oid_tarjeta"), this.getSesion());

			if (tarjeta == null)
				throw new ExceptionValidation(null, "Debe ingresar la Tarjeta");

			detTarConciEnt.setTipoTarjeta(tarjeta.getTipoTarjeta());
			detTarConciEnt.setTarjeta(tarjeta.getEquivIdentif());
			detTarConciEnt.save();
	...
	private IDataSet getDataSetNuevoRegCupon() {
		...
		dataset.create("TDetModifCuponTARJETA");
