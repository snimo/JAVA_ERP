...
public class TraerTalonariosAjuStk extends Operation {

  public TraerTalonariosAjuStk() {
  }
  
  public void execute(MapDatos mapaDatos) throws BaseException {
	    String tableName = mapaDatos.getString("nombreTabla");
	    IDataSet dsTalonario = getDataSetTalonario(tableName);
	    List talonarios = getTalonarios(mapaDatos);
  ...
  private List getTalonarios(MapDatos mapaDatos) throws BaseException {
  	if (mapaDatos.containsKey("rest_por_tipo_aju_y_oper_aju")) {
  		
  		String tipoAjuStk = mapaDatos.getString("TIPO_AJU_STK");
  		Integer oidTipoConfMov = mapaDatos.getInteger("OID_TIPO_CONF_MOV");
  		
  		return Talonario.getTalonariosByTipoAjuStkyOidTipoConfMov(tipoAjuStk,oidTipoConfMov,this.getSesion());
  	}
  	return Talonario.getTalonariosAjuStk(getSesion());
  }
