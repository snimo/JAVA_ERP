...
public class TraerEstrucFactTableEstadOC extends Operation {

  HashTableDatos fechas = new HashTableDatos();
	
  public TraerEstrucFactTableEstadOC() {
  }
  ...
  private void enviarDatosFactTable(MapDatos mapaDatos) throws BaseException {
  	...
  	IDataSet dsDimensiones = mapaDatos.getDataSet("TDimensiones");
  	//IDataSet ds = getDataSetFactTable(dsDimensiones);
  	...
  	TipoCambio tipoCambio = 
  		TipoCambio.getTipoCambioOC(this.getSesion());  	
  	
  	Iterator  iterEstadOC = EstadisticaOC.getEstadisticaOC(
  			mapaDatos.getDate("fec_desde"),
  			mapaDatos.getDate("fec_hasta"),
  			getListaClasif(dsDimensiones),
  			this.getSesion()).iterator();
  	while (iterEstadOC.hasNext()) {
  		EstadisticaOC estadisticaOC = (EstadisticaOC) iterEstadOC.next();
  		cargarEstadOC(dsFactTable,dsDimensiones,estadisticaOC,moneda,tipoCambio);
  	}
  ...
  private void cargarEstadOC(IDataSet dsFactTable,
  	...
  	dsFactTable.fieldValue("cantidad",estad.getCant());
  	
  	
  	CalculadorMoney importe = new CalculadorMoney(new Money(estad.getCant()));
  	importe.multiplicarPor(new Money(estad.getPrecio()));
  ...
  private void enviarEstructura(MapDatos mapaDatos) throws BaseException {
  	...
  	++secu;
  	tratarMetrica(ds,secu,"cantidad");
