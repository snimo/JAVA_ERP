...
public class TraerTalonariosSucursal extends Operation {

	public TraerTalonariosSucursal() {
	}

	public void execute(MapDatos mapaDatos) throws BaseException {

		String tableName = mapaDatos.getString("nombreTabla");
		String identifTC = mapaDatos.getString("identifTC");
		...
		List talonarios = Talonario.getTalonariosByIdentifTCySucursal(identifTC,sucursal,getSesion());
	...
	private void cargarRegistroTalonario(IDataSet dataset, Talonario talonario)
		...
		if (talonario.getImpresora_fiscal() != null)
			dataset.fieldValue("oid_imp_fiscal", talonario
					.getImpresora_fiscal().getOID());
		else
			dataset.fieldValue("oid_imp_fiscal", 0);
		if (talonario.getImpresora_comun() != null)
			dataset.fieldValue("oid_imp_comun", talonario.getImpresora_comun()
					.getOIDInteger());
		else
		...
		if (talonario.getNombre_archivo_impresion() != null)
			dataset.fieldValue("nom_arch_imp", talonario
					.getNombre_archivo_impresion());
		else
		...
		if (talonario.getFec_vto() != null)
			dataset.fieldValue("fec_vto", Fecha.getddmmyyyy(talonario
					.getFec_vto()));
		else
		...
		if (talonario.getUnidad_organizativa() != null)
			dataset.fieldValue("oid_uni_org", talonario
					.getUnidad_organizativa().getOIDInteger());
		else
