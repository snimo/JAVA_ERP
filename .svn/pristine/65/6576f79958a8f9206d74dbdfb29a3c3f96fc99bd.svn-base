...
import framework.applicarionServer.da.Varios.DBFecha;
...
//		Integer horaHasta = mapaDatos.getInteger("hora_hasta");
		...
		fecDesde = Fecha.getFecha(dia, mes, anio, 0, 0, 0);

		dia = Fecha.getDia(fecHasta);
		mes = Fecha.getMes(fecHasta);
		anio = Fecha.getAnio(fecHasta);

		fecHasta = Fecha.getFecha(dia, mes, anio, 23, 59, 59);
	...
	private void cargaDetallada(java.util.Date fecDesde, java.util.Date fecHasta, Legajo legajo) throws BaseException {
		...
		ProcesadorFichadasOficina procesador = new ProcesadorFichadasOficina(fecDesde, fecHasta, DBFecha.getFechaActual(this.getSesion()), ValorParametro.findByCodigoParametro(
				"CANT_MIN_NUEVA_JORNADA", this.getSesion()).getValorEntero(), auxFecHasta);
		List<RegistroFichada> registros = procesador.procesarFichadas(fichadas);
		
		Integer cantMinPersJD = ValorParametro.findByCodigoParametro("CANT_MIN_PERS_JD", this.getSesion()).getValorEntero();
		Integer cantMInPersJH = ValorParametro.findByCodigoParametro("CANT_MIN_PERS_JH", this.getSesion()).getValorEntero();
		Integer cantSalD = ValorParametro.findByCodigoParametro("CANT_SAL_D", this.getSesion()).getValorEntero();
		Integer cantSalH = ValorParametro.findByCodigoParametro("CANT_SAL_H", this.getSesion()).getValorEntero();
		Integer cantMinAfuD = ValorParametro.findByCodigoParametro("CANT_MIN_AFU_D", this.getSesion()).getValorEntero();
		Integer cantTMInAfuH = ValorParametro.findByCodigoParametro("CANT_MIN_AFU_H", this.getSesion()).getValorEntero();
		Integer cantPermOficD = ValorParametro.findByCodigoParametro("CANT_PERM_OFIC_D", this.getSesion()).getValorEntero();
		Integer cantPermOficH = ValorParametro.findByCodigoParametro("CANT_PERM_OFIC_H", this.getSesion()).getValorEntero();
		
		for (RegistroFichada registro : registros) {
			cargarRegistro(ds, registro, cantMinPersJD, cantMInPersJH, cantSalD, cantSalH, cantMinAfuD, cantTMInAfuH, cantPermOficD, cantPermOficH);
			for (CIPFichadaOficina fichada : registro.getFichadas()) {
				cargarFichada(dsDet, registro.getSecu(), fichada);
			}
		}
	...
	private IDataSet getDataSetCabecera() {
		...
		dataset.fieldDef(new Field("ok_permanencia_jornada", Field.STRING, 1));
		dataset.fieldDef(new Field("ok_cant_sal", Field.STRING, 1));
		dataset.fieldDef(new Field("ok_cant_min_afu", Field.STRING, 1));
		dataset.fieldDef(new Field("ok_perm_ofic", Field.STRING, 1));
		return dataset;
	}

	private void cargarRegistro(IDataSet dataset, RegistroFichada registro, Integer cantMinPersJD, Integer cantMInPersJH, Integer cantSalD, Integer cantSalH, Integer cantMinAfuD, Integer cantTMInAfuH, Integer cantPermOficD, Integer cantPermOficH) throws BaseException {
		dataset.append();
		...
		dataset.fieldValue("ok_permanencia_jornada", registro.getOKPermanenciaJornada(cantMinPersJD, cantMInPersJH));
		dataset.fieldValue("ok_cant_sal", registro.getOkCantSal(cantSalD, cantSalH));
		dataset.fieldValue("ok_cant_min_afu", registro.getOKCantMinAfu(cantMinAfuD, cantTMInAfuH));
		dataset.fieldValue("ok_perm_ofic", registro.getOKPermOfic(cantPermOficD, cantPermOficH));
	...
	private void cargarFichada(IDataSet dataset, Integer secu, CIPFichadaOficina fichada) throws BaseException {
		dataset.append();
		...
		dataset.fieldValue("observacion", fichada.getObservacion() != null ? fichada.getObservacion() : " ");
